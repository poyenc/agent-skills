...
Teammates didn't receive SessionStart hook output (directives, user profile, project config). Added `{{INITIAL_CONTEXT}}` placeholder filled from `.claude/teams/<name>/initial-context.md`. Lead copies hook output verbatim during setup (step 7b) and resume (step 3b). CLAUDE.md rules, skill listings, currentDate, and gitStatus are excluded — the system auto-injects those into subagents. **Status: done — verbatim hook output via file-based placeholder.**
