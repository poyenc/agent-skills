...
> - **Structure it as a short piece of writing, not a bag of facts.** A reader should be able to follow it start to finish without re-reading. The natural order is: (1) what's actually happening in the code, stated plainly; (2) why it's fine today / what makes the current behavior correct, if applicable; (3) what's missing or what changes that; (4) the exact, concrete fix — precisely where it goes, not just what kind of fix. Each part should earn its place by adding something the previous part didn't say — don't restate the location three different ways, don't pad with filler like "it's worth noting that" or "this is important because," and don't list disconnected observations with no throughline connecting them. If you can't explain why a sentence comes before or after its neighbor, cut or reorder it.
> - Pin down the fix to an exact location, not a vague "somewhere around here" — if the natural fix site isn't the line the comment is anchored to (e.g. the fix belongs in a different function or file), say exactly where, and why there rather than the more obvious-looking spot.
...
> - Any reference to a source file/line OTHER than the line this comment is anchored to (another function, a related test, a call site) must be a markdown hyperlink to the GitHub blob permalink at the exact reviewed commit SHA: `[label](https://github.com/{owner}/{repo}/blob/{commit_sha}/{path}#L{line})` (use `#L{start}-L{end}` for a range) — never a bare `file.py:123` reference in posted text. This applies even when the other reference sits in the same file or the same function as the anchor line (e.g. "the same fix applies at line 128" a few lines below the anchor) — proximity to the anchor doesn't exempt it from needing a link. If {REPO_OWNER}/{REPO_NAME} couldn't be resolved (no GitHub remote), fall back to plain `path:line` text instead.
...
5. If this skill's own rules changed (a `SKILL.md` edit) after any earlier comment in this same session was already posted, re-fetch and re-validate every already-posted comment against the current rules before considering the batch done — don't wait for a separate request to check. A rule tightened mid-session applies retroactively to what's already live, not just to what's posted after the edit. If an already-posted comment fails the new rules, show the user the drafted fix and get explicit approval before editing it (same as approval before any first-time post) — then apply it: inline comments via `gh api -X PATCH repos/{REPO_OWNER}/{REPO_NAME}/pulls/comments/{comment_id}`, standalone comments via `gh api -X PATCH repos/{REPO_OWNER}/{REPO_NAME}/issues/comments/{comment_id}`.
